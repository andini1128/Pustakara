...
<head>
    ...
    <title>Profil Admin - Pustakara</title>
    ...
    <style>
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .edit-link { margin-top:16px; display:inline-flex; align-items:center; justify-content:center; min-height:38px; border-radius:999px; padding:9px 15px; background:var(--blue); color:#fff; text-decoration:none; font-size:12px; font-weight:800; }
        .alert { margin-bottom:18px; border-radius:16px; padding:13px 15px; background:#eafaf1; color:#18794e; font-size:13px; font-weight:800; }
        ...
        @media (max-width:640px){ .content{padding:20px 16px;} .panel{padding:24px;} .summary-grid,.profile-grid,.sidebar-nav{grid-template-columns:1fr;} }
    </style>
</head>
<body>
    <div class="layout">
            ...
            <section class="panel">
                <div class="panel-header">
                    <div class="panel-title">
                        <h1>Profil Admin</h1>
                        <p>Data akun admin yang mengelola Pustakara.</p>
                    </div>
                </div>

                <div class="summary-grid">
                    <article class="summary-card"><div class="summary-value">{{ $stats['totalAdmins'] }}</div><div class="summary-label">Total admin</div></article>
                    ...
                    <div class="profile-grid">
                        @foreach ($adminUsers as $admin)
                            <article class="profile-card">
                                ...
                                <div class="info-list">
                                    <div class="info-item"><div class="info-label">Nama Pengguna</div><div class="info-value">{{ $admin->username }}</div></div>
                                    <div class="info-item"><div class="info-label">Surel</div><div class="info-value">{{ $admin->email }}</div></div>
                                    <div class="info-item"><div class="info-label">Alamat</div><div class="info-value">{{ $admin->alamat ?: 'Alamat belum diisi' }}</div></div>
                                    <div class="info-item"><div class="info-label">Terdaftar</div><div class="info-value">{{ optional($admin->created_at)->format('d M Y') ?: '-' }}</div></div>
                                </div>
                                <a href="{{ route('admin.profiles.edit', $admin) }}" class="edit-link">Edit Profil</a>
                            </article>
                        @endforeach
                    </div>
                @else
                    <div class="empty-state">Belum ada admin terdaftar.</div>
                @endif
            </section>
        </main>
    </div>
