...
<head>
    ...
    <title>Dasbor Admin - Pustakara</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .admin-pill { display: inline-flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 18px; background: rgba(255,255,255,.72); color: var(--navy); font-weight: 800; white-space: nowrap; }
        .admin-avatar { width: 48px; height: 48px; border-radius: 50%; background: var(--blue); color: #fff; display: inline-flex; align-items: center; justify-content: center; font-weight: 800; overflow: hidden; flex: 0 0 auto; }
        ...
        .table-wrap { padding: 14px; overflow-x: auto; border-radius: 18px; background: var(--soft); }
        table { width: 100%; min-width: 760px; border-collapse: collapse; border: 1px solid rgba(18,62,122,.14); background: #fff; }
        th, td { border: 1px solid rgba(18,62,122,.14); padding: 11px 12px; text-align: left; vertical-align: middle; }
        th { background: #eef2f5; color: var(--navy); font-size: 12px; font-weight: 800; text-transform: uppercase; }
        td { color: var(--ink); font-size: 12px; font-weight: 700; }
        .user-name { color: var(--navy); font-weight: 800; }
    ...
    <div class="layout">
        ...
        <main class="content">
            <section class="hero-card">
                <div>
                    <span class="hero-chip">Dasbor Admin</span>
                    <h1 class="hero-title">Pusat kendali Pustakara</h1>
                    <p class="hero-text">Kelola data buku, kategori, peminjaman, dan pengguna dari satu halaman yang ringkas.</p>
                </div>
            ...
            <section class="module-grid" aria-label="Menu utama admin">
                <a class="module-card" href="{{ route('books.index') }}">
                ...
                <a class="module-card" href="{{ route('books.categories') }}">
                    ...
                    <p class="module-desc">Atur kelompok buku agar koleksi mudah dicari dan dibaca.</p>
                    <span class="module-link">Kelola kategori</span>
                </a>
                <a class="module-card" href="{{ route('admin.loans') }}">
                ...
                <a class="module-card" href="{{ route('admin.users') }}">
                    <div class="module-top"><span class="module-icon"><svg viewBox="0 0 24 24" fill="none"><path d="M16 19C16 16.8 14.2 15 12 15H8C5.8 15 4 16.8 4 19" stroke-width="2"/><circle cx="10" cy="8" r="3" stroke-width="2"/></svg></span><span class="module-count">{{ $stats['totalUsers'] }}</span></div>
                    <h2 class="module-title">Pengguna</h2>
                    <p class="module-desc">Lihat akun admin, petugas, dan anggota yang terdaftar.</p>
                    <span class="module-link">Kelola pengguna</span>
                </a>
            </section>
            ...
            <section class="overview-grid">
                <div class="panel">
                    <div class="panel-header">
                        ...
                        <a class="panel-link" href="{{ route('books.index') }}">Lihat semua</a>
                    </div>
                    @if ($books->isNotEmpty())
                        <div class="list">
                ...
                <div class="panel">
                    <div class="panel-header">
                        <h2 class="panel-title">Akun Pengguna</h2>
                        <a class="panel-link" href="{{ route('admin.users') }}">Lihat semua</a>
                    </div>
                    @php
                        $memberUsers = $users->where('role', 'anggota')->values();
                    @endphp
                                ...
                                <tbody>
                                    @foreach ($memberUsers as $user)
                                        <tr>
                                            ...
                                            <td>{{ $user->username }}</td>
                                            <td>{{ $user->email }}</td>
                                            <td class="user-muted">{{ $user->alamat ?: 'Belum diisi' }}</td>
                    ...
                    @else
                        <div class="empty-state">Belum ada akun pengguna terdaftar.</div>
                    @endif
                </div>
            </section>
        </main>
