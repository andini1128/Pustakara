...
<head>
    ...
    <title>Data Peminjaman Petugas - Pustakara</title>
...
<body>
    <div class="layout">
        @include('partials.staff-sidebar', ['activeMenu' => 'loans'])

        <main class="content">
            <section class="panel">
                <div class="panel-header">
                    <div class="panel-title">
                        <h1>Data Peminjaman Petugas</h1>
                                        ...
                                        <td>
                                            <div class="action-group">
                                                @if ($status === 'Menunggu Persetujuan')
                                                    <form method="POST" action="{{ route('staff.loans.approve', $loan) }}">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" class="action-btn approve">Setujui</button>
                                                    </form>
                                                    <form method="POST" action="{{ route('staff.loans.reject', $loan) }}">
                                                        @csrf
                                                        @method('PATCH')
                                                ...
                                                <form method="POST" action="{{ route('staff.loans.destroy', $loan) }}" onsubmit="return confirm('Hapus data peminjaman ini?')">
                                                    @csrf
                                                    @method('DELETE')
